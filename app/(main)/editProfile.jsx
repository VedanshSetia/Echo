...
const EditProfile = () => {
  ...
  if (user?.image) {
    if (typeof user.image === 'object' && user.image.uri) {
      ...
      console.log("Setting image from object:", imageSource);
    } else if (typeof user.image === 'string') {
      const imageUrl = getUserImageSrc(user.image);
      // Fix here - handle when getUserImageSrc returns an object with uri property
      if (typeof imageUrl === 'string') {
        imageSource = { uri: imageUrl };
      } else if (imageUrl && imageUrl.uri) {
        imageSource = { uri: imageUrl.uri };
      }
      console.log("Image URL after fix:", imageSource);
    }
  }
          ...
          <View style={styles.avatarContainer}>
            {imageSource ? (
              ...
              <View style={[styles.avatar, {backgroundColor: '#E1E1E1', justifyContent: 'center', alignItems: 'center'}]}>
                <Icon name="user" size={40} color="#999999" />
              </View>
            )}
